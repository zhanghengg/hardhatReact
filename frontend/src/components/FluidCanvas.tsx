...
export function FluidCanvas({ className = '' }: FluidCanvasProps) {
  ...
  const initFluidSimulation = useCallback((canvas: HTMLCanvasElement, isDarkMode: boolean) => {
    // 配置
    const config: Config = {
      SIM_RESOLUTION: 128,
      DYE_RESOLUTION: 1024,
      ...
      PAUSED: false,
      BACK_COLOR: isDarkMode ? { r: 0, g: 0, b: 0 } : { r: 255, g: 255, b: 255 },
      TRANSPARENT: true,
      BLOOM: true,
      BLOOM_ITERATIONS: 8,
    ...
    // 创建 dithering 纹理 (使用程序生成代替加载图片)
    const ditheringTexture = createDitheringTexture();

    function createDitheringTexture(): TextureObject {
      const texture = gl.createTexture()!;
      gl.bindTexture(gl.TEXTURE_2D, texture);
    ...
    function getTextureScale(texture: TextureObject, width: number, height: number): { x: number; y: number } {
      return {
  ...
  useEffect(() => {
    ...
    const cleanup = initFluidSimulation(canvas, isDark);
