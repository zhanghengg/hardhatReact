...
export const clearAbiGroup = async (context, config) => {
...
export const exportAbiGroup = async (context, config) => {
    const outputDirectory = path.resolve(context.config.paths.root, config.path);
    if (outputDirectory === context.config.paths.root) {
        throw new HardhatPluginError(pkg.name, 'resolved path must not be root directory');
    }
    const outputData = [];
    const fullNames = Array.from(await context.artifacts.getAllFullyQualifiedNames());
    await Promise.all(fullNames.map(async (fullName) => {
        ...
        if (config.except.length && config.except.some((m) => fullName.match(m)))
            return;
        let { abi, sourceName, contractName } = await context.artifacts.readArtifact(fullName);
        if (!abi.length)
            return;
        ...
        const destination = path.resolve(outputDirectory, config.rename(sourceName, contractName)) +
            extension;
        outputData.push({ destination, contents });
    }));
    outputData.reduce((acc, { destination, contents }) => {
        const previousContents = acc[destination];
        if (previousContents && previousContents !== contents) {
            throw new HardhatPluginError(pkg.name, `multiple distinct contracts share same output destination: ${destination}`);
        }
    ...
    await Promise.all(outputData.map(async ({ destination, contents }) => {
